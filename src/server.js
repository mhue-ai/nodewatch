...
db.exec(`
  CREATE TABLE IF NOT EXISTS users (
    address TEXT PRIMARY KEY, created_at TEXT DEFAULT (datetime('now')), last_login TEXT
  );
  CREATE TABLE IF NOT EXISTS nodes (
    id INTEGER PRIMARY KEY AUTOINCREMENT, owner TEXT NOT NULL REFERENCES users(address) ON DELETE CASCADE,
    name TEXT NOT NULL, type TEXT NOT NULL CHECK(type IN ('guardian','synaptron','collector','geocore')),
    nft_id TEXT, guid TEXT, host TEXT NOT NULL, port INTEGER NOT NULL, docker_name TEXT DEFAULT '-',
    created_at TEXT DEFAULT (datetime('now')), UNIQUE(owner, name)
  );
  CREATE TABLE IF NOT EXISTS checks (
    id INTEGER PRIMARY KEY AUTOINCREMENT, node_id INTEGER NOT NULL REFERENCES nodes(id) ON DELETE CASCADE,
    checked_at TEXT DEFAULT (datetime('now')), status TEXT NOT NULL CHECK(status IN ('up','down')),
    latency_ms INTEGER DEFAULT -1, error TEXT
  );
  CREATE TABLE IF NOT EXISTS validator_snapshots (
    id INTEGER PRIMARY KEY AUTOINCREMENT, validator_addr TEXT NOT NULL,
    checked_at TEXT DEFAULT (datetime('now')), moniker TEXT, status TEXT,
    jailed INTEGER DEFAULT 0, tokens TEXT, commission_rate TEXT,
...
function auth(req, res, next) {
  ...
  req.userAddress = s.address; next();
}
...
app.post('/api/auth/verify', async (req, res) => {
  try {
    ...
    db.prepare(`INSERT INTO users (address,last_login) VALUES (?,datetime('now')) ON CONFLICT(address) DO UPDATE SET last_login=datetime('now')`).run(address);
    const token = crypto.randomBytes(48).toString('hex');
    sessions.set(token, { address, expires: Date.now() + 86400000 });
    res.json({ token, address });
  } catch (err) { res.status(500).json({ error: 'Auth failed' }); }
});

app.get('/api/auth/session', auth, (req, res) => res.json({ address: req.userAddress }));

// ── Nodes CRUD ────────────────────────────────────────────────
app.get('/api/nodes', auth, (req, res) => res.json(db.prepare('SELECT * FROM nodes WHERE owner=? ORDER BY type,name').all(req.userAddress)));
...
app.post('/api/nodes', auth, (req, res) => {
  ...
  try {
    const r = db.prepare('INSERT INTO nodes (owner,name,type,nft_id,guid,host,port,docker_name) VALUES (?,?,?,?,?,?,?,?)').run(req.userAddress,name,type,nft_id||null,guid||null,host,parseInt(port),docker_name||'-');
...
app.put('/api/nodes/:id', auth, (req, res) => {
  const n = db.prepare('SELECT * FROM nodes WHERE id=? AND owner=?').get(req.params.id, req.userAddress);
...
app.delete('/api/nodes/:id', auth, (req, res) => {
  const r = db.prepare('DELETE FROM nodes WHERE id=? AND owner=?').run(req.params.id, req.userAddress);
  r.changes ? res.json({ deleted:true }) : res.status(404).json({ error: 'Not found' });
});

// ── Node Status & History ─────────────────────────────────────
app.get('/api/status', auth, (req, res) => {
  const nodes = db.prepare('SELECT * FROM nodes WHERE owner=?').all(req.userAddress);
...
app.get('/api/history', auth, (req, res) => {
  ...
  res.json(db.prepare('SELECT id,name,type FROM nodes WHERE owner=?').all(req.userAddress).map(n => ({
...
// ══════════════════════════════════════════════════════════════
// STAKING & VALIDATOR MONITORING
// ══════════════════════════════════════════════════════════════

// My validators: delegations + rewards + validator health in one call
app.get('/api/staking/my-validators', auth, async (req, res) => {
  try {
    const [delData, rewData] = await Promise.all([
      lcdFetch(`/cosmos/staking/v1beta1/delegations/${req.userAddress}`).catch(()=>({delegation_responses:[]})),
      lcdFetch(`/cosmos/distribution/v1beta1/delegators/${req.userAddress}/rewards`).catch(()=>({rewards:[],total:[]}))
    ]);
    ...
    const results = await Promise.all(delegations.map(async del => {
    ...
    res.json({ address: req.userAddress, total_pending_rewards: rewData.total?.[0]?.amount||'0', delegations: results });
  } catch(err) {
    console.error('[staking] my-validators error:', err.message);
    res.json({ address: req.userAddress, total_pending_rewards:'0', delegations:[] });
  }
});

// All validators (for comparison)
app.get('/api/staking/validators', async (_, res) => {
  try {
    const d = await lcdFetch('/cosmos/staking/v1beta1/validators?status=BOND_STATUS_BONDED&pagination.limit=200');
    res.json(d);
  } catch(e) { res.json({ validators:[] }); }
});

// Validator health history from DB
...
// NFT proxy
app.get('/api/neutaro/nfts/:address', async (req, res) => {
  try { res.json(await lcdFetch(`/cosmos/nft/v1beta1/nfts?owner=${req.params.address}`)); }
...
async function runValidatorChecks() {
  ...
  try {
    ...
    let sigMap = {};
    try {
      const s = await lcdFetch('/cosmos/slashing/v1beta1/signing_infos?pagination.limit=500');
      for (const i of (s.info||[])) sigMap[i.address] = parseInt(i.missed_blocks_counter||'0');
    } catch(e){}

    for (const v of all) {
      const missed = 0; // consensus→operator mapping requires bech32 conversion; we track overall
  ...
  } catch(e) { console.error('[validators] Error:', e.message); }
}
